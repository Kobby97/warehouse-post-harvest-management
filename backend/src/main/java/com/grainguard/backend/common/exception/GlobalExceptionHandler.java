...
/**
 * One place for every controller's error responses, so the frontend always
 * gets the same JSON shape regardless of which endpoint failed:
 * { timestamp, status, error, message } (validation errors additionally
 * include a fieldErrors map).
 */
